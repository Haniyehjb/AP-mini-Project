...
public:
    ...
    player();
    player(player obj);
    player(string _id,string _name,string _using_gun_id,Guns _using_gun_name,int _health,int _armor,bool _AI);
...
void set_using_gun_name(Guns _using_gun_name) {
...
player::player(string _id,string _name,string _using_gun_id,Guns _using_gun_name,int _health,int _armor,bool _AI int _money){
    id = _id;
    name = _name;
    using_gun_id = _using_gun_id;
    using_gun_name = _using_gun_name;
...
bool operator==(const player& obj) {
    if (id == obj.id && name == obj.name /* و بقیه شرایط */)
        return true; // ✅
    return false; // اضافه کردن مقدار بازگشتی پیش‌فرض
}
...
void terrorist::set_using_bomb(bool _using_bomb){bomb = _using_bomb;}
...
terrorist::terrorist(terrorist obj){
    bomb = obj.bomb;
    using_bomb = obj.using_bomb;
}
terrorist::terrorist(bool _bomb,bool _using_bomb){
    bomb = _bomb;
...
string game_map::get_maker_map(){return maker_name;}
...
int display_history(const string& filename , const string& userid){
    ...
    while(getline(file , line)){
        for(char c : line){
            if(c == '0'){
                winner = "Terrorist";
                for(int i = 0 ; i < team_Terrorist.size() ; i++){
...
void game_manager::start_game(){
    ...
    int ctindex = 0 ;
    while (terroristindex < terrorist.size() && ctindex < ct.size()) {
        player& terrorist = team_terrorist[terroristindex];
        // بقیه کدها
    } // ✅
    player& ct = team_ct[ctindex];


    double terroristPower = terrorist->getMoney();
    double ctPower = ct->getMoney();


    for (int gunId : terrorist->getWeaponIds()) {
...
bool users::registerUser (){
    ...
    cin>>passwordHashed;
    passwordhssh = hashpassword(passwordHashed);
    saveToFile();
    return true;
}
...
#include "main3.cpp" // فرض کن فایلت اسمش main3.cpp هست
...
void test_map_creation() {
    ...
    if (m.set_map_name() == "Dust2")
        cout << "✅ test_map_creation passed\n";
    else
        cout << "❌ test_map_creation failed\n";
}

// ===== تست users (ثبت نام و ورود) =====
void test_user_register_and_login() {
    users u;
    u.registerUser();
    bool login_success = u.loginUser("your_password_here"); // پسورد تستی
    if (login_success)
        cout << "✅ test_user_login passed\n";
    else
        cout << "❌ test_user_login failed\n";
}
void test_full_game_flow() {
    game_manager* gm = game_manager::instance();
    gm->setting_menu(); // فرض کن دستی اجرا شده یا مقادیر ثابت شده

    gm->start_game();   // بازی رو شروع می‌کنیم

    int result = gm->set_game_result();
    if (result == 0 || result == 1 || result == -1)
        cout << "✅ test_full_game_flow passed\n";
    else
        cout << "❌ test_full_game_flow failed\n";
}
int main() {
    ...
    test_map_creation();
    // test_user_register_and_login(); // اینو موقع دیباگ واقعی دستی فعال کن

    cout << "\n🚀 Running Integration Tests:" << endl;

    //test_full_game_flow(); // وقتی همه چیز آماده شد فعال کن
