#include <iostream>
#include <vector>
#include <string>
#include <windows.h>
#include <rpc.h>
#include <cstdlib>
#include <fstream>
#include <functional>
#include <random>
#include <thread>
#include <cassert>
#include <ctime>
#include <windows.h>
#include <wincrypt.h>
#include <sstream>
#include <iomanip>
using namespace std;
bool isAuthenticated = false;
int wins;


// تابع هش SHA-256 با استفاده از Cryptography API ویندوز
string sha256(const string& str) {
    HCRYPTPROV hProv = 0;
    HCRYPTHASH hHash = 0;
    BYTE rgbHash[32];
    DWORD cbHash = 32;
    CHAR rgbDigits[] = "0123456789abcdef";
    if (!CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_AES, CRYPT_VERIFYCONTEXT)) {
        return "";
    }
    if (!CryptCreateHash(hProv, CALG_SHA_256, 0, 0, &hHash)) {
        CryptReleaseContext(hProv, 0);
        return "";
    }
    if (!CryptHashData(hHash, (BYTE*)str.c_str(), str.size(), 0)) {
        CryptDestroyHash(hHash);
        CryptReleaseContext(hProv, 0);
        return "";
    }
    if (!CryptGetHashParam(hHash, HP_HASHVAL, rgbHash, &cbHash, 0)) {
        CryptDestroyHash(hHash);
        CryptReleaseContext(hProv, 0);
        return "";
    }
    stringstream ss;
    for (DWORD i = 0; i < cbHash; i++) {
        ss << rgbDigits[rgbHash[i] >> 4];
        ss << rgbDigits[rgbHash[i] & 0xf];
    }
    CryptDestroyHash(hHash);
    CryptReleaseContext(hProv, 0);
    return ss.str();
}

#pragma comment(lib, "rpcrt4.lib")

enum Guns {
    AK47 = 1, //1000,20,5
    M4A1, ///2000,20 10
    AWP,//3000 , 25 10
    P90,//4000 25 20
    UMP45,//5000 30 20
    FAMAS,//6000 30 25
    DESERT_EAGLE,//7000 35 25
    GLOCK,//8000 35 30
    MP5,//9000 40 30
    SHOTGUN///10000 40 40
};

using namespace std;

void switch_menu(int cmd);

string generate_id() {
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<> dis(0, 15);
    const char* hex_chars = "0123456789abcdef";
    string uuid;
    for (int i = 0; i < 32; ++i) {
        uuid += hex_chars[dis(gen)];
    }
    return uuid;
}

string hashPassword(const string& password) {
    return to_string(hash<string>{}(password));
}

int setting_before_start = 0;

// Forward declarations
class player;
class terrorist;
class ct;
class gun;
class game_map;
class game_manager;
class users;
class node;

class gun {
private:
    int num_shot;
    int price;
    string gun_id;
    Guns gun_name;
    int power_shot;
    int num_gun;

public:
    void set_num_shot(int);
    void set_price(int);
    void set_gun_id(string);
    void set_gun_name(Guns);
    void set_power_shot(int);
    void set_num_gun(int);
    int get_num_shot();
    int get_price();
    string get_gun_id();
    Guns get_gun_name();
    int get_power_shot();
    int get_num_gun();
    gun();
    gun(const gun& obj);
    gun(int _num_shot, int _price, string _gun_id, Guns _gun_name, int _power_shot, int _num_gun);
    ~gun();
    gun& operator=(const gun& obj);
    bool operator!=(const gun& obj);
    bool operator==(const gun& obj);
    string getGunName(Guns gun);
    // در کلاس gun این تابع عمومی را اضافه کنید
    void initializeGunStats(Guns gunType) {
        switch (gunType) {
        case AK47:
            set_num_shot(30);    // تعداد گلوله
            set_power_shot(36);  // قدرت هر شات
            set_price(2700);     // قیمت
            break;
        case M4A1:
            set_num_shot(30);
            set_power_shot(31);
            set_price(3100);
            break;
        case AWP:
            set_num_shot(10);
            set_power_shot(110); // AWP بسیار قوی است
            set_price(4750);
            break;
        case P90:
            set_num_shot(50);    // ظرفیت زیاد
            set_power_shot(21);  // قدرت متوسط
            set_price(2350);
            break;
        case UMP45:
            set_num_shot(25);
            set_power_shot(25);
            set_price(1200);
            break;
        case FAMAS:
            set_num_shot(25);
            set_power_shot(25);
            set_price(2050);
            break;
        case DESERT_EAGLE:
            set_num_shot(7);     // خشاب کم
            set_power_shot(63);  // قدرت بالا
            set_price(700);
            break;
        case GLOCK:
            set_num_shot(20);
            set_power_shot(11);  // قدرت پایین
            set_price(200);
            break;
        case MP5:
            set_num_shot(30);
            set_power_shot(26);
            set_price(1500);
            break;
        case SHOTGUN:
            set_num_shot(8);     // تعداد کم ولی پرقدرت
            set_power_shot(40);  // قدرت در هر شات
            set_price(1200);
            break;
        default:
            // مقادیر پیش‌فرض برای اسلحه‌های ناشناخته
            set_num_shot(0);
            set_power_shot(0);
            set_price(0);
            break;
        }
        set_gun_name(gunType); // تنظیم نام اسلحه
        set_gun_id("GUN_" + to_string(static_cast<int>(gunType))); // تولید ID منحصر به فرد
    }
};

class player {
private:
    string id;
    string name;
    string using_gun_id;
    Guns using_gun_name;
    int health;
    int armor;
    bool AI;
    int money;
    int score;
    node* phead;

public:
    player(string playerName, int initialScore = 0);
    void set_id(string);
    void set_name(string);
    void set_using_gun_id(string);
    void set_using_gun_name(Guns);
    void set_health(int);
    void set_armor(int);
    void set_AI(bool);
    void set_money(int);
    string get_id();
    string get_name();
    string get_using_gun_id();
    Guns get_using_gun_name();
    int get_health();
    int get_armor();
    bool get_AI();
    int get_money();
    int get_score();
    player();
    player(const player& obj);
    player(string _id, string _name, string _using_gun_id, Guns _using_gun_name, int _health, int _armor, bool _AI, int _money);
    player& operator=(const player& obj);
    bool operator!=(const player& obj);
    bool operator==(const player& obj);
    node* add_node(const player& p);
    void delete_node(Guns _guns);
    double calculate_power() {
        double power = 0;
        //        // اگر تفنگ دارد، قدرت تفنگ را هم اضافه می‌کنیم
        //        if (using_gun_name != Guns{}) { // فرض می‌کنیم Guns{} مقدار پیش‌فرض (نامشخص) است
        //            gun g; // اینجا باید اطلاعات تفنگ را از یک منبع (مثلا لیست تفنگ‌ها) بگیریم
        //            power += g.get_num_shot() * g.get_power_shot();
        //        }
        if (money >= 1000 && money < 2000) {
            money -= 1000;
            power += 100;
        }
        else if (money >= 2000 && money < 3000) {
            money -= 2000;
            power += 200;
        }
        else if (money >= 3000 && money < 4000) {
            money -= 3000;
            power += 250;
        }
        else if (money >= 4000 && money < 5000) {
            money -= 4000;
            power += 500;
        }
        else if (money >= 5000 && money < 6000) {
            money -= 5000;
            power += 600;
        }
        else if (money >= 6000 && money < 7000) {
            money -= 6000;
            power += 750;
        }
        else if (money >= 7000 && money < 8000) {
            money -= 7000;
            power += 875;
        }
        else if (money >= 8000 && money < 9000) {
            money -= 8000;
            power += 1050;
        }
        else if (money >= 9000 && money < 10000) {
            money -= 2000;
            power += 1200;
        }
        else if (money >= 10000) {
            money -= 10000;
            power += 1600;
        }
        else {
            power += 0;
        }
        power += money;
        return power;
    }
    int get_health() const { return health; }
};

class node {
public:
    node* pprv;
    node* pnxt;
    player data;
};

node* phead = nullptr;

class terrorist : public player {
private:
    bool bomb;
    bool using_bomb;

public:
    void set_bomb(bool);
    void set_using_bomb(bool);
    bool get_bomb();
    bool get_using_bomb();
    terrorist();
    terrorist(const terrorist& obj);
    terrorist(bool _bomb, bool _using_bomb);
    terrorist& operator=(const terrorist& obj);
    bool operator!=(const terrorist& obj);
    bool operator==(const terrorist& obj);
};

class ct : public player {
private:
    bool kit;
    bool cooling_bomb;

public:
    void set_kit(bool);
    void set_cooling_bomb(bool);
    bool get_kit();
    bool get_cooling_bomb();
    ct();
    ct(const ct& obj);
    ct(bool _kit, bool _cooling_bomb);
    ct& operator=(const ct& obj);
    bool operator!=(const ct& obj);
    bool operator==(const ct& obj);
};

class game_map {
private:
    string map_name;
    string maker_map;
    bool am_pm;

public:
    void set_map_name(string);
    void set_maker_map(string);
    void set_am_pm(bool);
    string get_map_name();
    string get_maker_map();
    bool get_am_pm();
    game_map();
    game_map(const game_map& obj);
    game_map(string _map_name, string _maker_map, bool _am_pm);
    game_map& operator=(const game_map& obj);
    bool operator!=(const game_map& obj);
    bool operator==(const game_map& obj);
    void checkWinner();
};

class game_manager {
private:
    vector<player> team_ct;       // نه team_ct یا TeamCT
    vector<player> team_terrorist; // نه team_terrorist یا TeamTerrorist
    int game_result;
    string map_name;
    string currentUsername;

    static game_manager* _instance;
    int terroristIndex; // اضافه شده
    int ctIndex;

    game_manager() : map_name(""), game_result(0) {}
    void decideTieBreaker();

public:
    static game_manager* instance();
    void set_map_name(string);
    void set_game_result(int);
    string get_map_name();
    int get_game_result();
    game_manager(const game_manager& obj);
    game_manager(string _map_name, int _game_result);
    game_manager& operator=(const game_manager& obj);
    bool operator!=(const game_manager& obj);
    bool operator==(const game_manager& obj);
    int display_history(const string& username);
    void setting_menu();
    void start_game();
    void checkWinner();
    void add_ct_player(const player& p) { team_ct.push_back(p); }
    void add_terrorist_player(const player& p) { team_terrorist.push_back(p); }
    void setCurrentUsername(const string& username) { currentUsername = username; }
};

game_manager* game_manager::_instance = nullptr;

game_manager* game_manager::instance() {
    if (_instance == nullptr) {
        _instance = new game_manager();
    }
    return _instance;
}

class users {
private:
    string username;
    string nickname;
    string passwordHash;
    int wins;

public:
    users();
    users(const users& other);
    users(string _username, string _nickname, string _passwordHash, int _wins);
    bool registerUser();
    bool loginUser();
    void updateWins();
    void saveToFile();
    static users loadFromFile(const string& username);
    string getusername() const;
    string getnickname() const;
    int getWins() const;
};

// Implementations of gun class
void gun::set_num_shot(int _num_shot) { num_shot = _num_shot; }
void gun::set_price(int _price) { price = _price; }
void gun::set_gun_id(string _gun_id) { gun_id = _gun_id; }
void gun::set_gun_name(Guns _gun_name) { gun_name = _gun_name; }
void gun::set_power_shot(int _power_shot) { power_shot = _power_shot; }
void gun::set_num_gun(int _num_gun) { num_gun = _num_gun; }
int gun::get_num_shot() { return num_shot; }
int gun::get_price() { return price; }
string gun::get_gun_id() { return gun_id; }
Guns gun::get_gun_name() { return gun_name; }
int gun::get_power_shot() { return power_shot; }
int gun::get_num_gun() { return num_gun; }
gun::gun() : num_shot(0), price(0), gun_id(""), gun_name(AK47), power_shot(0), num_gun(0) {}
gun::gun(const gun& obj) : num_shot(obj.num_shot), price(obj.price), gun_id(obj.gun_id),
gun_name(obj.gun_name), power_shot(obj.power_shot), num_gun(obj.num_gun) {
}
gun::gun(int _num_shot, int _price, string _gun_id, Guns _gun_name, int _power_shot, int _num_gun) :
    num_shot(_num_shot), price(_price), gun_id(_gun_id), gun_name(_gun_name),
    power_shot(_power_shot), num_gun(_num_gun) {
}
gun& gun::operator=(const gun& obj) {
    num_shot = obj.num_shot;
    price = obj.price;
    gun_id = obj.gun_id;
    gun_name = obj.gun_name;
    power_shot = obj.power_shot;
    num_gun = obj.num_gun;
    return *this;
}
bool gun::operator!=(const gun& obj) { return !(*this == obj); }
bool gun::operator==(const gun& obj) {
    return num_shot == obj.num_shot &&
        price == obj.price &&
        gun_id == obj.gun_id &&
        gun_name == obj.gun_name &&
        power_shot == obj.power_shot &&
        num_gun == obj.num_gun;
}
gun::~gun() { num_gun--; }
string gun::getGunName(Guns gun) {
    switch (gun) {
    case AK47: return "AK-47";
    case M4A1: return "M4A1";
    case AWP: return "AWP";
    case P90: return "P90";
    case UMP45: return "UMP-45";
    case FAMAS: return "FAMAS";
    case DESERT_EAGLE: return "Desert Eagle";
    case GLOCK: return "Glock";
    case MP5: return "MP5";
    case SHOTGUN: return "Shotgun";
    default: return "Unknown Gun";
    }
}

// Implementations of player class
player::player(string playerName, int initialScore) : name(playerName), score(initialScore) {
    id = "P" + generate_id();
}
void player::set_id(string _id) { id = _id; }
void player::set_name(string _name) { name = _name; }
void player::set_using_gun_id(string _using_gun_id) { using_gun_id = _using_gun_id; }
void player::set_using_gun_name(Guns _using_gun_name) { using_gun_name = _using_gun_name; }
void player::set_health(int _health) { health = _health; }
void player::set_armor(int _armor) { armor = _armor; }
void player::set_AI(bool _ai) { AI = _ai; }
void player::set_money(int _money) { money = _money; }
string player::get_id() { return id; }
string player::get_name() { return name; }
string player::get_using_gun_id() { return using_gun_id; }
Guns player::get_using_gun_name() { return using_gun_name; }
int player::get_health() { return health; }
int player::get_armor() { return armor; }
bool player::get_AI() { return AI; }
int player::get_money() { return money; }
int player::get_score() { return score; }
player::player() : id(""), name(""), using_gun_id(""), using_gun_name(AK47),
health(0), armor(0), money(0), AI(false), score(0), phead(nullptr) {
}
player::player(const player& obj) : id(obj.id), name(obj.name), using_gun_id(obj.using_gun_id),
using_gun_name(obj.using_gun_name), health(obj.health), armor(obj.armor),
AI(obj.AI), money(obj.money), score(obj.score), phead(nullptr) {
}
player::player(string _id, string _name, string _using_gun_id, Guns _using_gun_name,
    int _health, int _armor, bool _AI, int _money) :
    id(_id), name(_name), using_gun_id(_using_gun_id), using_gun_name(_using_gun_name),
    health(_health), armor(_armor), AI(_AI), money(_money), score(0), phead(nullptr) {
}
player& player::operator=(const player& obj) {
    id = obj.id;
    name = obj.name;
    using_gun_id = obj.using_gun_id;
    using_gun_name = obj.using_gun_name;
    health = obj.health;
    armor = obj.armor;
    AI = obj.AI;
    money = obj.money;
    score = obj.score;
    return *this;
}
bool player::operator!=(const player& obj) { return !(*this == obj); }
bool player::operator==(const player& obj) {
    return id == obj.id &&
        name == obj.name &&
        using_gun_id == obj.using_gun_id &&
        using_gun_name == obj.using_gun_name &&
        health == obj.health &&
        armor == obj.armor &&
        AI == obj.AI &&
        money == obj.money &&
        score == obj.score;
}
node* player::add_node(const player& p) {
    node* pnew = new node;
    pnew->data = p;
    pnew->pnxt = nullptr;
    pnew->pprv = nullptr;
    if (!phead) {
        phead = pnew;
        pnew->pnxt = pnew;
        pnew->pprv = pnew;
    }
    else {
        node* last = phead->pprv;
        last->pnxt = pnew;
        pnew->pprv = last;
        pnew->pnxt = phead;
        phead->pprv = pnew;
    }
    return pnew;
}
void player::delete_node(Guns _guns) {
    if (phead == nullptr) {
        cout << "Error: No member to delete." << endl;
        return;
    }
    node* ptmp = phead;
    do {
        if (ptmp->data.get_using_gun_name() == _guns) {
            if (ptmp == phead) {
                if (ptmp->pnxt == phead) {
                    delete ptmp;
                    phead = nullptr;
                }
                else {
                    node* last = phead->pprv;
                    phead = ptmp->pnxt;
                    last->pnxt = phead;
                    phead->pprv = last;
                    delete ptmp;
                }
            }
            else {
                node* before = ptmp->pprv;
                node* after = ptmp->pnxt;
                before->pnxt = after;
                after->pprv = before;
                delete ptmp;
            }
            return;
        }
        ptmp = ptmp->pnxt;
    } while (ptmp != phead);
    cout << "Not found." << endl;
}

// Implementations of terrorist class
void terrorist::set_bomb(bool _bomb) { bomb = _bomb; }
void terrorist::set_using_bomb(bool _using_bomb) { using_bomb = _using_bomb; }
bool terrorist::get_bomb() { return bomb; }
bool terrorist::get_using_bomb() { return using_bomb; }
terrorist::terrorist() : bomb(true), using_bomb(false) {}
terrorist::terrorist(const terrorist& obj) : player(obj), bomb(obj.bomb), using_bomb(obj.using_bomb) {}
terrorist::terrorist(bool _bomb, bool _using_bomb) : bomb(_bomb), using_bomb(_using_bomb) {}
terrorist& terrorist::operator=(const terrorist& obj) {
    player::operator=(obj);
    bomb = obj.bomb;
    using_bomb = obj.using_bomb;
    return *this;
}
bool terrorist::operator!=(const terrorist& obj) { return !(*this == obj); }
bool terrorist::operator==(const terrorist& obj) {
    return bomb == obj.bomb && using_bomb == obj.using_bomb;
}

// Implementations of ct class
void ct::set_kit(bool _kit) { kit = _kit; }
void ct::set_cooling_bomb(bool _cooling_bomb) { cooling_bomb = _cooling_bomb; }
bool ct::get_kit() { return kit; }
bool ct::get_cooling_bomb() { return cooling_bomb; }
ct::ct() : kit(true), cooling_bomb(false) {}
ct::ct(const ct& obj) : player(obj), kit(obj.kit), cooling_bomb(obj.cooling_bomb) {}
ct::ct(bool _kit, bool _cooling_bomb) : kit(_kit), cooling_bomb(_cooling_bomb) {}
ct& ct::operator=(const ct& obj) {
    player::operator=(obj);
    kit = obj.kit;
    cooling_bomb = obj.cooling_bomb;
    return *this;
}
bool ct::operator!=(const ct& obj) { return !(*this == obj); }
bool ct::operator==(const ct& obj) {
    return kit == obj.kit && cooling_bomb == obj.cooling_bomb;
}

// Implementations of game_map class
void game_map::set_map_name(string _map_name) { map_name = _map_name; }
void game_map::set_maker_map(string _maker_map) { maker_map = _maker_map; }
void game_map::set_am_pm(bool _am_pm) { am_pm = _am_pm; }
string game_map::get_map_name() { return map_name; }
string game_map::get_maker_map() { return maker_map; }
bool game_map::get_am_pm() { return am_pm; }
game_map::game_map() : map_name(""), maker_map(""), am_pm(true) {}
game_map::game_map(const game_map& obj) : map_name(obj.map_name), maker_map(obj.maker_map), am_pm(obj.am_pm) {}
game_map::game_map(string _map_name, string _maker_map, bool _am_pm) :
    map_name(_map_name), maker_map(_maker_map), am_pm(_am_pm) {
}
game_map& game_map::operator=(const game_map& obj) {
    map_name = obj.map_name;
    maker_map = obj.maker_map;
    am_pm = obj.am_pm;
    return *this;
}
bool game_map::operator!=(const game_map& obj) { return !(*this == obj); }
bool game_map::operator==(const game_map& obj) {
    return map_name == obj.map_name &&
        maker_map == obj.maker_map &&
        am_pm == obj.am_pm;
}

// Implementations of game_manager class
void game_manager::set_map_name(string _map_name) { map_name = _map_name; }
void game_manager::set_game_result(int _game_result) { game_result = _game_result; }
string game_manager::get_map_name() { return map_name; }
int game_manager::get_game_result() { return game_result; }
game_manager::game_manager(const game_manager& obj) : map_name(obj.map_name), game_result(obj.game_result) {}
game_manager::game_manager(string _map_name, int _game_result) : map_name(_map_name), game_result(_game_result) {}
game_manager& game_manager::operator=(const game_manager& obj) {
    map_name = obj.map_name;
    game_result = obj.game_result;
    return *this;
}
bool game_manager::operator!=(const game_manager& obj) { return !(*this == obj); }
bool game_manager::operator==(const game_manager& obj) {
    return map_name == obj.map_name && game_result == obj.game_result;
}
//int game_manager::display_history(const string& filename, const string& userid) {
//    string line, winner;
//    ifstream file(filename);
//    if (!file.is_open()) {
//        cerr << "Error: can not open file!" << endl;
//        return -1;
//    }
//    int round_game = 1;
//    int user_wins = 0;
//    while (getline(file, line)) {
//        for (char c : line) {
//            if (c == '0') {
//                winner = "Terrorist";
//                for (size_t i = 0; i < team_terrorist.size(); i++) {
//                    if (team_terrorist[i].get_id() == userid) {
//                        user_wins++;
//                    }
//                    round_game++;
//                }
//            }
//            else if (c == '1') {
//                winner = "CT";
//                for (size_t i = 0; i < team_ct.size(); ++i) {
//                    if (team_ct[i].get_id() == userid) {
//                        user_wins++;
//                    }
//                    round_game++;
//                }
//            }
//            else {
//                cout << "invalid result" << endl;
//                continue;
//            }
//        }
//    }
//    cout << "Round " << round_game << ": " << "winner :" << winner << endl;
//    return user_wins;
//}
int game_manager::display_history(const string& username) {
    // مسیر کامل فایل کاربران
    const string user_file = "C:\\Users\\JahanBakhsh\\Desktop\\users.txt";

    // 1. ابتدا کاربر را در فایل users.txt پیدا کنیم
    ifstream user_db(user_file);
    if (!user_db.is_open()) {
        cerr << "Error: Cannot open user database file!" << endl;
        return -1;
    }

    string stored_user, stored_nickname, stored_passwordHash;
    int stored_wins = 0;
    bool user_found = false;

    // جستجوی کاربر در فایل
    while (user_db >> stored_user >> stored_nickname >> stored_passwordHash >> stored_wins) {
        if (stored_user == username) {
            user_found = true;
            break;
        }
    }
    user_db.close();

    if (!user_found) {
        cout << "User '" << username << "' not found!" << endl;
        return -1;
    }

    // 2. نمایش تاریخچه بازی‌های کاربر
    cout << "\n===== Game History for " << username << " =====" << endl;
    cout << "Total wins: " << stored_wins << endl;

    // 3. خواندن فایل تاریخچه بازی‌ها (اگر وجود دارد)
    const string history_file = "C:\\Users\\JahanBakhsh\\Desktop\\game_history.txt";
    ifstream history_db(history_file);
    if (!history_db.is_open()) {
        cout << "No game history available yet." << endl;
        return stored_wins;
    }

    string line;
    int round_count = 0;
    int user_wins = 0;

    while (getline(history_db, line)) {
        round_count++;
        if (line.find(username) != string::npos) {
            size_t pos = line.find("winner:");
            if (pos != string::npos) {
                string winner = line.substr(pos + 7);
                cout << "Round " << round_count << ": " << username << " was in the winning team (" << winner << ")" << endl;
                user_wins++;
            }
        }
    }

    history_db.close();
    return user_wins;
}
void game_manager::setting_menu() {
    int a;
    int c;
    int t;
    cout << "please choose one map" << endl;
    cout << "1.map1" << endl << "2.map2" << endl << "3.map3" << endl
        << "4.map4" << endl << "5.map5" << endl;
    cin >> a;

    do {
        system("cls");
        cout << "please enter the number of player of each team" << endl;
        cout << "CT team" << endl;
        cin >> c;
        cout << "Terrorist team" << endl;
        cin >> t;
    } while (!((c / (float)t >= 0.95 && c / (float)t <= 1.05) ||
        (t / (float)c >= 0.95 && t / (float)c <= 1.05)));

    system("cls");

    // پاک کردن تیم‌های قبلی
    team_ct.clear();
    team_terrorist.clear();

    // ایجاد بازیکنان CT
    cout << "please enter money of each CT player " << endl;
    for (int i = 0; i < c; ++i) {
        int mon;
        cin >> mon;
        player p("CT_Player" + to_string(i + 1));
        p.set_money(mon);
        p.set_health(100);  // سلامت اولیه
        p.set_armor(50);    // زره اولیه
        team_ct.push_back(p);
    }

    // ایجاد بازیکنان تروریست
    cout << "please enter money of each terrorist player " << endl;
    for (int i = 0; i < t; ++i) {
        int mon;
        cin >> mon;
        player p("Terrorist_Player" + to_string(i + 1));
        p.set_money(mon);
        p.set_health(100);  // سلامت اولیه
        p.set_armor(50);    // زره اولیه
        team_terrorist.push_back(p);

    }
    setting_before_start = 1;
    start_game();
}
void game_manager::checkWinner() {
    bool terroristTeamAlive = false;
    bool ctTeamAlive = false;

    for (const auto& player : team_terrorist) {
        if (player.get_health() > 0) {
            terroristTeamAlive = true;
            break;
        }
    }

    for (const auto& player : team_ct) {
        if (player.get_health() > 0) {
            ctTeamAlive = true;
            break;
        }
    }

    if (!terroristTeamAlive && ctTeamAlive) {
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); // آبی
        cout << "CT team wins by eliminating all terrorists!" << endl;
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
        game_result = 1;

        // افزایش تعداد بردهای کاربر اگر در تیم CT باشد
        if (!currentUsername.empty()) {
            // مسیر کامل فایل کاربران
            const string user_file = "C:\\Users\\JahanBakhsh\\Desktop\\users.txt";

            // 1. خواندن تمام خطوط فایل
            ifstream inFile(user_file);
            vector<string> lines;
            string line;

            while (getline(inFile, line)) {
                lines.push_back(line);
            }
            inFile.close();

            // 2. پیدا کردن کاربر و افزایش تعداد بردها
            ofstream outFile(user_file);
            for (const auto& l : lines) {
                istringstream iss(l);
                string username, nickname, passwordHash;
                int wins;

                if (iss >> username >> nickname >> passwordHash >> wins) {
                    if (username == currentUsername) {
                        wins++; // افزایش تعداد بردها
                        outFile << username << " " << nickname << " " << passwordHash << " " << wins << endl;
                    }
                    else {
                        outFile << l << endl;
                    }
                }
            }
            outFile.close();
        }
        return;
    }
    else if (!ctTeamAlive && terroristTeamAlive) {
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12); // قرمز
        cout << "Terrorist team wins by eliminating all CTs!" << endl;
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
        game_result = 0;
        return;
    }
    else if (!terroristTeamAlive && !ctTeamAlive) {
        decideTieBreaker();
        return;
    }

    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 10); // سبز
    cout << "Round completed. Both teams still have alive players." << endl;
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
}
//void game_manager::setting_menu() {
//    int a;
//    int c;
//    int t;
//    cout << "please choose one map" << endl;
//    cout << "1.map1" << endl << "2.map2" << endl << "3.map3" << endl
//        << "4.map4" << endl << "5.map5" << endl;
//    cin >> a;
//    do {
//        system("cls");
//        cout << "please enter the number of player of each team" << endl;
//        cout << "CT team" << endl;
//        cin >> c;
//        cout << "Terrorist team" << endl;
//        cin >> t;
//    } while (!((c / (float)t >= 0.95 && c / (float)t <= 1.05) ||
//        (t / (float)c >= 0.95 && t / (float)c <= 1.05)));
//    system("cls");
//    cout << "please enter money of each CT player " << endl;
//    vector<int> ct_money(c);
//    int mon;
//    for (int i = 0; i < c; ++i) {
//        cin >> mon;
//        ct_money.push_back(mon);
//    }
//    cout << "please enter money of each terrorist player " << endl;
//    vector<int> terrorist_money(t);
//    int mon2;
//    for (int i = 0; i < t; ++i) {
//        cin >> mon2;
//        terrorist_money.push_back(mon2);
//    }
//    setting_before_start = 1;
//}
//
//void game_manager::start_game() {
//    int terroristIndex = 0;
//    int ctIndex = 0;
//    int round = 1;
//    while (terroristIndex < team_terrorist.size() && ctIndex < team_ct.size()) {
//        system("cls");
//        cout << "Round " << round++ << endl << endl;
//        player& terroristPlayer = team_terrorist[terroristIndex];
//        player& ctPlayer = team_ct[ctIndex];
//        // محاسبه قدرت هر بازیکن
//        double terroristPower = terroristPlayer.calculate_power();
//        double ctPower = ctPlayer.calculate_power();
//        // نمایش اطلاعات بازیکنان
//        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12); // قرمز برای تروریست
//        cout << "Terrorist " << terroristPlayer.get_name() << " (Power: " << terroristPower << ")";
//        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); // آبی برای CT
//        cout << " vs CT " << ctPlayer.get_name() << " (Power: " << ctPower << ")" << endl;
//        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید برای متن معمولی
//        if (terroristPower > ctPower) {
//            // تروریست قوی‌تر است
//            int damage = terroristPower - ctPower;
//            ctPlayer.set_health(ctPlayer.get_health() - damage);
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12); // قرمز
//            cout << "Terrorist " << terroristPlayer.get_name();
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
//            cout << " defeated ";
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); // آبی
//            cout << "CT " << ctPlayer.get_name();
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
//            cout << " with " << damage << " damage!" << endl;
//            if (ctPlayer.get_health() <= 0) {
//                cout << "CT " << ctPlayer.get_name() << " has been eliminated!" << endl;
//                ctIndex++;
//            }
//        }
//        else if (ctPower > terroristPower) {
//            // CT قوی‌تر است
//            int damage = ctPower - terroristPower;
//            terroristPlayer.set_health(terroristPlayer.get_health() - damage);
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); // آبی
//            cout << "CT " << ctPlayer.get_name();
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
//            cout << " defeated ";
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12); // قرمز
//            cout << "Terrorist " << terroristPlayer.get_name();
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
//            cout << " with " << damage << " damage!" << endl;
//            if (terroristPlayer.get_health() <= 0) {
//                cout << "Terrorist " << terroristPlayer.get_name() << " has been eliminated!" << endl;
//                terroristIndex++;
//            }
//        }
//        else {
//            // قدرت برابر است
//            terroristPlayer.set_health(terroristPlayer.get_health() - 1);
//            ctPlayer.set_health(ctPlayer.get_health() - 1);
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12); // قرمز
//            cout << "Terrorist " << terroristPlayer.get_name();
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
//            cout << " and ";
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); // آبی
//            cout << "CT " << ctPlayer.get_name();
//            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
//            cout << " both took 1 damage!" << endl;
//            if (terroristPlayer.get_health() <= 0) {
//                cout << "Terrorist " << terroristPlayer.get_name() << " has been eliminated!" << endl;
//                terroristIndex++;
//            }
//            if (ctPlayer.get_health() <= 0) {
//                cout << "CT " << ctPlayer.get_name() << " has been eliminated!" << endl;
//                ctIndex++;
//            }
//        }
//        // نمایش وضعیت فعلی
//        cout << "Current Status:" << endl;
//        cout << "Terrorists: " << (team_terrorist.size() - terroristIndex) << " players remaining" << endl;
//        cout << "CTs: " << (team_ct.size() - ctIndex) << " players remaining" << endl;
//        this_thread::sleep_for(chrono::milliseconds(500)); // تاخیر نیم ثانیه
//    }
//    checkWinner();
//}
////
//void game_manager::checkWinner() {
//    // بررسی وضعیت فعلی تیم‌ها
//    bool terroristTeamAlive = false;
//    bool ctTeamAlive = false;
//
//    // بررسی آیا تیم تروریست بازیکن زنده دارد
//    for (const auto& player : team_terrorist) {
//        if (player.get_health() > 0) {
//            terroristTeamAlive = true;
//            break;
//        }
//    }
//
//    // بررسی آیا تیم CT بازیکن زنده دارد
//    for (const auto& player : team_ct) {
//        if (player.get_health() > 0) {
//            ctTeamAlive = true;
//            break;
//        }
//    }
//
//    // حالت ۱: یکی از تیم‌ها کاملاً حذف شده
//    if (!terroristTeamAlive && ctTeamAlive) {
//        cout << "CT team wins by eliminating all terrorists!" << endl;
//        game_result = 1;
//        return;
//    }
//    else if (!ctTeamAlive && terroristTeamAlive) {
//        cout << "Terrorist team wins by eliminating all CTs!" << endl;
//        game_result = 0;
//        return;
//    }
//    // حالت ۲: هر دو تیم کاملاً حذف شده‌اند
//    else if (!terroristTeamAlive && !ctTeamAlive) {
//        // تصمیم‌گیری بر اساس معیارهای ثانویه
//        decideTieBreaker();
//        return;
//    }
//
//    // اگر بازی هنوز ادامه دارد
//    cout << "Round completed. Both teams still have alive players." << endl;
//}
//
//void game_manager::decideTieBreaker() {
//    // محاسبه مجموع قدرت باقی‌مانده هر تیم
//    double terroristTotalPower = 0;
//    double ctTotalPower = 0;
//
//    for (const auto& player : team_terrorist) {
//        if (player.get_health() > 0) {
//            terroristTotalPower += player.calculate_power();
//        }
//    }
//
//    for (const auto& player : team_ct) {
//        if (player.get_health() > 0) {
//            ctTotalPower += player.calculate_power();
//        }
//    }
//
//    // مقایسه قدرت کلی تیم‌ها
//    if (terroristTotalPower > ctTotalPower) {
//        cout << "Terrorist team wins by having more total power ("
//            << terroristTotalPower << " vs " << ctTotalPower << ")!" << endl;
//        game_result = 0;
//    }
//    else if (ctTotalPower > terroristTotalPower) {
//        cout << "CT team wins by having more total power ("
//            << ctTotalPower << " vs " << terroristTotalPower << ")!" << endl;
//        game_result = 1;
//    }
//    else {
//        // اگر قدرت کلی هم مساوی بود، بر اساس تعداد بازیکنان زنده
//        int aliveTerrorists = 0;
//        int aliveCTs = 0;
//
//        for (const auto& player : team_terrorist) {
//            if (player.get_health() > 0) aliveTerrorists++;
//        }
//
//        for (const auto& player : team_ct) {
//            if (player.get_health() > 0) aliveCTs++;
//        }
//
//        if (aliveTerrorists > aliveCTs) {
//            cout << "Terrorist team wins by having more alive players ("
//                << aliveTerrorists << " vs " << aliveCTs << ")!" << endl;
//            game_result = 0;
//        }
//        else if (aliveCTs > aliveTerrorists) {
//            cout << "CT team wins by having more alive players ("
//                << aliveCTs << " vs " << aliveTerrorists << ")!" << endl;
//            game_result = 1;
//        }
//        else {
//            // اگر همه چیز مساوی بود، تصمیم تصادفی
//            srand(time(nullptr));
//            if (rand() % 2 == 0) {
//                cout << "Terrorist team wins by random decision!" << endl;
//                game_result = 0;
//            }
//            else {
//                cout << "CT team wins by random decision!" << endl;
//                game_result = 1;
//            }
//        }
//    }
//}

void game_manager::start_game() {
    int terroristIndex = 0;
    int ctIndex = 0;
    int round = 1;
    while (terroristIndex < team_terrorist.size() && ctIndex < team_ct.size()) {
        //system("cls");
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 14); //yellow color
        cout << "Round " << round++ << endl << endl;
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // white color

        player& terroristPlayer = team_terrorist[terroristIndex];
        player& ctPlayer = team_ct[ctIndex];


        double terroristPower = terroristPlayer.calculate_power();
        double ctPower = ctPlayer.calculate_power();


        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12); //red color
        cout << "Terrorist " << terroristPlayer.get_name();
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8); // gray
        cout << " (Power: ";
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12); //red
        cout << terroristPower;
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8); // gray
        cout << ")";

        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // white
        cout << " vs ";

        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); // blue
        cout << "CT " << ctPlayer.get_name();
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8); // gray
        cout << " (Power: ";
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); // blue
        cout << ctPower;
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8); // gray
        cout << ")" << endl;
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); //white

        if (terroristPower > ctPower) {

            int damage = terroristPower - ctPower;
            ctPlayer.set_health(ctPlayer.get_health() - damage);

            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12);
            cout << "Terrorist " << terroristPlayer.get_name();
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
            cout << " defeated ";
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9);
            cout << "CT " << ctPlayer.get_name();
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
            cout << " with ";
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12);
            cout << damage;
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
            cout << " damage!" << endl;

            if (ctPlayer.get_health() <= 0) {
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9);
                cout << "CT " << ctPlayer.get_name();
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
                cout << " has been eliminated!" << endl;
                ctIndex++;
            }
        }
        else if (ctPower > terroristPower) {

            int damage = ctPower - terroristPower;
            terroristPlayer.set_health(terroristPlayer.get_health() - damage);

            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9);
            cout << "CT " << ctPlayer.get_name();
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
            cout << " defeated ";
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12);
            cout << "Terrorist " << terroristPlayer.get_name();
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
            cout << " with ";
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9);
            cout << damage;
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
            cout << " damage!" << endl;

            if (terroristPlayer.get_health() <= 0) {
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12);
                cout << "Terrorist " << terroristPlayer.get_name();
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
                cout << " has been eliminated!" << endl;
                terroristIndex++;
            }
        }
        else {

            terroristPlayer.set_health(terroristPlayer.get_health() - 1);
            ctPlayer.set_health(ctPlayer.get_health() - 1);

            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12);
            cout << "Terrorist " << terroristPlayer.get_name();
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
            cout << " and ";
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9);
            cout << "CT " << ctPlayer.get_name();
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
            cout << " both took ";
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 14);
            cout << "1";
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
            cout << " damage!" << endl;

            if (terroristPlayer.get_health() <= 0) {
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12);
                cout << "Terrorist " << terroristPlayer.get_name();
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
                cout << " has been eliminated!" << endl;
                terroristIndex++;
            }
            if (ctPlayer.get_health() <= 0) {
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9);
                cout << "CT " << ctPlayer.get_name();
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
                cout << " has been eliminated!" << endl;
                ctIndex++;
            }
        }

        // نمایش وضعیت فعلی
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 11);
        cout << "Current Status:" << endl;
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12);
        cout << "Terrorists: " << (team_terrorist.size() - terroristIndex) << " players remaining" << endl;
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9);
        cout << "CTs: " << (team_ct.size() - ctIndex) << " players remaining" << endl;
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);

        this_thread::sleep_for(chrono::milliseconds(500));
    }
    checkWinner();
}

//void game_manager::checkWinner() {
//    bool terroristTeamAlive = false;
//    bool ctTeamAlive = false;
//
//    for (const auto& player : team_terrorist) {
//        if (player.get_health() > 0) {
//            terroristTeamAlive = true;
//            break;
//        }
//    }
//
//    for (const auto& player : team_ct) {
//        if (player.get_health() > 0) {
//            ctTeamAlive = true;
//            break;
//        }
//    }
//
//    if (!terroristTeamAlive && ctTeamAlive) {
//        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); // آبی
//        cout << "CT team wins by eliminating all terrorists!" << endl;
//        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
//        game_result = 1;
//
//        return;
//    }
//    else if (!ctTeamAlive && terroristTeamAlive) {
//        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12); // قرمز
//        cout << "Terrorist team wins by eliminating all CTs!" << endl;
//        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
//        game_result = 0;
//        return;
//    }
//    else if (!terroristTeamAlive && !ctTeamAlive) {
//        decideTieBreaker();
//        return;
//    }
//
//    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 10); // سبز
//    cout << "Round completed. Both teams still have alive players." << endl;
//    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
//}

void game_manager::decideTieBreaker() {
    double terroristTotalPower = 0;
    double ctTotalPower = 0;

    for (auto& player : team_terrorist) {
        if (player.get_health() > 0) {
            terroristTotalPower += player.calculate_power();
        }
    }

    for (auto& player : team_ct) {
        if (player.get_health() > 0) {
            ctTotalPower += player.calculate_power();
        }
    }

    if (terroristTotalPower > ctTotalPower) {
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12); // قرمز
        cout << "Terrorist team wins by having more total power ("
            << terroristTotalPower << " vs " << ctTotalPower << ")!" << endl;
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
        game_result = 0;
    }
    else if (ctTotalPower > terroristTotalPower) {
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); // آبی
        cout << "CT team wins by having more total power ("
            << ctTotalPower << " vs " << terroristTotalPower << ")!" << endl;
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
        game_result = 1;
    }
    else {
        int aliveTerrorists = 0;
        int aliveCTs = 0;

        for (const auto& player : team_terrorist) {
            if (player.get_health() > 0) aliveTerrorists++;
        }

        for (const auto& player : team_ct) {
            if (player.get_health() > 0) aliveCTs++;
        }

        if (aliveTerrorists > aliveCTs) {
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12); // قرمز
            cout << "Terrorist team wins by having more alive players ("
                << aliveTerrorists << " vs " << aliveCTs << ")!" << endl;
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
            game_result = 0;
        }
        else if (aliveCTs > aliveTerrorists) {
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); // آبی
            cout << "CT team wins by having more alive players ("
                << aliveCTs << " vs " << aliveTerrorists << ")!" << endl;
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
            game_result = 1;
        }
        else {
            srand(time(nullptr));
            if (rand() % 2 == 0) {
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12); // قرمز
                cout << "Terrorist team wins by random decision!" << endl;
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
                game_result = 0;
            }
            else {
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); // آبی
                cout << "CT team wins by random decision!" << endl;
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7); // سفید
                game_result = 1;
            }
        }
    }
}

// Implementations of users class
users::users() : username(""), nickname(""), passwordHash(""), wins(0) {}
users::users(const users& other) : username(other.username), nickname(other.nickname),
passwordHash(other.passwordHash), wins(other.wins) {
}
users::users(string _username, string _nickname, string _passwordHash, int _wins) :
    username(_username), nickname(_nickname), passwordHash(_passwordHash), wins(_wins) {
}
bool users::registerUser() {
    cout << "please enter your username" << endl;
    cin >> username;
    cout << "please enter your nickname" << endl;
    cin >> nickname;
    cout << "enter your password" << endl;
    string password;
    cin >> password;
    passwordHash = sha256(password);
    saveToFile();
    return true;
}
//bool users::loginUser() {
//    string input_username, input_password;
//    cout << "Enter username: ";
//    cin >> input_username;
//    cout << "Enter password: ";
//    cin >> input_password;
//    users loadedUser = users::loadFromFile(input_username);
//    if (loadedUser.getusername().empty() || loadedUser.passwordHash != sha256(input_password)) {
//        cout << "Invalid username or password!" << endl;
//        isAuthenticated = false;
//        return false;
//    }
//    this->username = loadedUser.username;
//    this->nickname = loadedUser.nickname;
//    this->passwordHash = loadedUser.passwordHash;
//    this->wins = loadedUser.wins;
//    cout << "Login successful! Welcome " << this->nickname << endl;
//    isAuthenticated = true;
//    return true;
//}
//bool users::loginUser() {
//    string input_username, input_password;
//    cout << "Enter username: ";
//    cin >> input_username;
//    cout << "Enter password: ";
//    cin >> input_password;
//
//    users loadedUser = users::loadFromFile(input_username);
//
//    if (loadedUser.getusername().empty() || loadedUser.passwordHash != sha256(input_password)) {
//        cout << " Invalid username or password!" << endl;
//        isAuthenticated = false;
//        return false;
//    }
//
//    // Update current user data
//    this->username = loadedUser.username;
//    this->nickname = loadedUser.nickname;
//    this->passwordHash = loadedUser.passwordHash;
//    this->wins = loadedUser.wins;
//
//    cout << " Login successful! Welcome, " << this->nickname << "!" << endl;
//    isAuthenticated = true;
//    return true;
//}
bool users::loginUser() {
    string input_username, input_password;
    cout << "Enter username: ";
    cin >> input_username;
    cout << "Enter password: ";
    cin >> input_password;

    users loadedUser = users::loadFromFile(input_username);

    if (loadedUser.getusername().empty() || loadedUser.passwordHash != sha256(input_password)) {
        cout << "Invalid username or password!" << endl;
        isAuthenticated = false;
        return false;
    }

    // Update current user data
    this->username = loadedUser.username;
    this->nickname = loadedUser.nickname;
    this->passwordHash = loadedUser.passwordHash;
    this->wins = loadedUser.wins;

    // ذخیره نام کاربر در game_manager
    game_manager::instance()->setCurrentUsername(this->username);

    cout << "Login successful! Welcome " << this->nickname << endl;
    isAuthenticated = true;
    return true;
}
void users::updateWins() {
    wins++;
}
//void users::saveToFile() {
//    ofstream file("users.txt", ios::app);
//    if (file.is_open()) {
//        file << username << " " << nickname << " " << passwordHash << " " << wins << endl;
//        file.close();
//    }
//    else {
//        cerr << "Unable to open file for writing." << endl;
//    }
//}
void users::saveToFile() {
    ofstream file("C:\\Users\\JahanBakhsh\\Desktop\\users.txt", ios::app);
    if (file.is_open()) {
        file << username << " " << nickname << " " << passwordHash << " " << wins << endl;
        file.close();
        cout << "User data saved successfully!" << endl;  // پیام موفقیت
    }
    else {
        cerr << "Unable to open file for writing." << endl;
    }
}
//users users::loadFromFile(const string& username) {
//    ifstream file("users.txt");
//    string user, nickname, passwordHash;
//    int wins;
//    while (file >> user >> nickname >> passwordHash >> wins) {
//        if (user == username) {
//            return users(user, nickname, passwordHash, wins);
//        }
//    }
//    return users();
//}
users users::loadFromFile(const string& username) {
    ifstream file("C:\\Users\\JahanBakhsh\\Desktop\\users.txt");  // مسیر کامل فایل
    string user, nickname, passwordHash;
    int wins;

    while (file >> user >> nickname >> passwordHash >> wins) {
        if (user == username) {
            file.close();  // بستن فایل قبل از خروج
            return users(user, nickname, passwordHash, wins);
        }
    }

    file.close();  // بستن فایل در صورت عدم یافتن کاربر
    return users();  // بازگشت شیء خالی اگر کاربر یافت نشد
}
string users::getusername() const { return username; }
string users::getnickname() const { return nickname; }
int users::getWins() const { return wins; }

void login_menu() {
    int a;
    cout << "1. Log in" << endl;
    cout << "2. Sign up" << endl;
    cout << "Enter your choice: ";
    cin >> a;
    users user;
    switch (a) {
    case 1:
        user.loginUser();
        break;
    case 2:

        user.registerUser();
        break;
    default:
        cout << "Invalid choice!" << endl;
        break;
    }
}

void switch_menu(int cmd) {
    game_manager* gam = game_manager::instance();
    switch (cmd) {
    case 1: // شروع بازی
        if (setting_before_start == 0) {

            cout << "Please set up the game first using option 2." << endl;
        }
        else {
            gam->start_game();
        }
        break;
    case 2: // تنظیمات بازی
        system("cls");
        gam->setting_menu();
        break;
    case 3: { // تاریخچه بازی
        system("cls");
        string  userid;
        cout << "Enter your name: ";
        cin >> userid;
        int result = gam->display_history(userid);
        if (result != -1) {
            cout << "You have won " << result << " rounds!" << endl;
        }
        break;
    }
    case 4: { // اطلاعات کاربر
        system("cls");
        string username;
        cout << "Enter your username: ";
        cin >> username;
        users user = users::loadFromFile(username);
        if (user.getusername() != "") {
            cout << "Username: " << user.getusername() << endl;
            cout << "Nickname: " << user.getnickname() << endl;
            cout << "Wins: " << user.getWins() << endl;
        }
        else {
            cout << "User not found!" << endl;
        }
        break;
    }
    case 5: // خروج
        exit(0);
    default: // ورودی نامعتبر
        cout << "Invalid command. Please try again." << endl;
        break;
    }
}

// تابع اصلی برنامه
//int main() {
//    login_menu();
//    while (true) {
//
//
//        if (isAuthenticated = true) {
//            int cmd;
//            cout << "1⚪ START GAME" << endl;
//            cout << "2⚪ GAME SETTING" << endl;
//            cout << "3⚪ HISTORY" << endl;
//            cout << "4⚪ INFORMATION" << endl;
//            cout << "5⚪ EXIT" << endl;
//            cin >> cmd;
//            switch_menu(cmd); // فراخوانی تابع منو}
//        }
//    }
//    return 0;
//}
//int main() {
//    // Display the login menu and authenticate the user
//    login_menu();
//
//    // Main game loop
//    while (true) {
//        // Check if the user is authenticated
//        if (isAuthenticated) {
//            int cmd;
//            // Display the main menu options
//            cout << "\n--- Main Menu ---" << endl;
//            cout << "1. START GAME" << endl;
//            cout << "2. GAME SETTING" << endl;
//            cout << "3. HISTORY" << endl;
//            cout << "4. INFORMATION" << endl;
//            cout << "5. EXIT" << endl;
//            cout << "Please enter your choice: ";
//            cin >> cmd;
//
//            // Call the appropriate function based on user input
//            switch_menu(cmd);
//        }
//        else {
//            cout << "You must be logged in to access the game menu." << endl;
//            login_menu(); // Prompt the user to log in again if not authenticated
//        }
//    }
//    return 0;
//}

int main() {
    // تنظیمات اولیه کنسول برای نمایش بهتر
    //SetConsoleTitle(L"Counter-Strike Game Manager");
    HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
    SetConsoleTextAttribute(hConsole, 11); // رنگ آبی روشن برای عنوان

    cout << "====================================" << endl;
    cout << "   COUNTER-STRIKE GAME MANAGER" << endl;
    cout << "====================================" << endl << endl;

    SetConsoleTextAttribute(hConsole, 7); // برگشت به رنگ سفید
    login_menu();


    //   while (!isAuthenticated) {
//        try {
//            login_menu();
//
//            if (!isAuthenticated) {
//                SetConsoleTextAttribute(hConsole, 12); // رنگ قرمز برای خطا
//                cout << "Authentication failed. Please try again." << endl;
//                SetConsoleTextAttribute(hConsole, 7);
//                this_thread::sleep_for(chrono::seconds(1));
//                system("cls");
//            }
//        }
//        catch (const exception& e) {
//            cerr << "Error during authentication: " << e.what() << endl;
//            return 1;
//        }
//    }

    // حلقه اصلی برنامه
    while (true) {

        system("cls");
        SetConsoleTextAttribute(hConsole, 14); // رنگ زرد برای منو

        cout << "\n=== MAIN MENU ===" << endl;
        cout << "1. Start New Game" << endl;
        cout << "2. Game Settings" << endl;
        cout << "3. View Game History" << endl;
        cout << "4. Player Information" << endl;
        cout << "5. Exit" << endl;
        cout << "=================" << endl;

        SetConsoleTextAttribute(hConsole, 10); // رنگ سبز برای ورودی
        // cout << "\nEnter your choice (1-5): ";

        int choice;
        cin >> choice;
        switch_menu(choice);

        // اعتبارسنجی ورودی
//            if (cin.fail() || choice < 1 || choice > 5) {
//                cin.clear();
//                //cin.ignore(numeric_limits<streamsize>::max(), '\n');
//                SetConsoleTextAttribute(hConsole, 12);
//                cout << "Invalid input! Please enter a number between 1 and 5." << endl;
//                SetConsoleTextAttribute(hConsole, 7);
//                this_thread::sleep_for(chrono::seconds(1));
//                continue;
//            }
//
//            SetConsoleTextAttribute(hConsole, 7); // رنگ سفید
//            switch_menu(choice);
//
//            // مکث کوتاه قبل از پاک کردن صفحه
//            if (choice != 5) {
//                cout << "\nPress Enter to continue...";
//                cin.ignore();
//                cin.get();
//            }
//
//        }
//        catch (const exception& e) {
//            SetConsoleTextAttribute(hConsole, 12);
//            cerr << "\nAn error occurred: " << e.what() << endl;
//            SetConsoleTextAttribute(hConsole, 7);
//            cout << "Press Enter to continue...";
//            cin.ignore();
//            cin.get();
//        }
//    }

        return 0;
    }
}
//enum Guns {
//    AK47 = 1, //1000,20,5
//    M4A1, ///2000,20 10
//    AWP,//3000 , 25 10
//    P90,//4000 25 20
//    UMP45,//5000 30 20
//    FAMAS,//6000 30 25
//    DESERT_EAGLE,//7000 35 25
//    GLOCK,//8000 35 30
//    MP5,//9000 40 30
//    SHOTGUN///10000 40 40
//};
//int culculate_power_by_money(int& money){
//    int buygun;
//    if(money >= 1000 && money <2000){
//        money-=1000;
//        buygun = 100;
//    }
//    else if(money >= 2000 && money <3000){
//        money-=2000;
//        buygun = 200;
//    }
//    else if(money >= 3000 && money <4000){
//        money-=3000;
//        buygun = 250;
//    }
//    else if(money >= 4000 && money <5000){
//        money-=4000;
//        buygun = 500;
//    }
//    else if(money >= 5000 && money <6000){
//        money-=5000;
//        buygun = 600;
//    }
//    else if(money >= 6000 && money <7000){
//        money-=6000;
//        buygun = 750;
//    }
//    else if(money >= 7000 && money <8000){
//        money-=7000;
//        buygun = 875;
//    }
//    else if(money >= 8000 && money <9000){
//        money-=8000;
//        buygun = 1050;
//    }
//    else if(money >= 9000 && money <10000){
//        money-=2000;
//        buygun = 1200;
//    }
//    else if(money >= 10000 ){
//        money-=10000;
//        buygun = 1600;
//    }
//    else{
//        buygun = 0;
//    }
//    return buygun;
//
//}
